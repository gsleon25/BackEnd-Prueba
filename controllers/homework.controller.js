...
function setHomework(req, res){
    ...
    if(userId != req.user.sub){
        ...
        Schedule.findById(userId, (err, userFind)=>{
            if(err){
                return res.status(500).send({message: 'Error genral de la busqueda'});
            }else if(userFind){
                homework.nameHomework = params.nameHomework,
                homework.description = params.description,
                homework.urgencyLevel = params.urgencyLevel,
                homework.importanceLevel = params.importanceLevel

                homework.save((err , tareaGuardado)=>{
                    if(err){
                        return res.status(500).send({message: 'Error General al guardar la Tarea'});

                    }else if(tareaGuardado){
                        Schedule.findByIdAndUpdate(userId, {$push:{homework: tareaGuardado._id}}, {new: true},(err, pushtarea)=>{
                            if(err){
                                return res.status(500).send({message: 'Error General al setear tarea'});
                            }else if(pushtarea){
                                return res.send({mensaje: 'Tarea creada y agregado', pushtarea});
                            }else{
                                return res.status(404).send({message: 'No se seteo la tarea, pero si se creo en la DB'});

                            }
                        });
                    }else{
                        return res.status(404).send({message: 'No se pudo Guardar la tarea'});
                    }
                })
            }else{
...
function saveHomework(req,res) {
    ...
    if(params.nameHomework && params.description){
        ...
        tareaModel.schedule = params.schedule;
